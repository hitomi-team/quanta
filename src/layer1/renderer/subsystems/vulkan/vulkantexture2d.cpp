...
bool CVulkanTexture2D::SetData(unsigned char *data, unsigned width, unsigned height, Renderer::SamplerStateDesc samplerstatedesc)
{
	...
	image->TransitionLayout(0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, g_vulkanCtx->queue_family_indices[0], g_vulkanCtx->queue_family_indices[0]);
	image->CopyBufferToImage(buf);
	image->TransitionLayout(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, g_vulkanCtx->queue_family_indices[0], g_vulkanCtx->queue_family_indices[0]);
